...
public class RequiresFeatureAuthorizationHandlerTests
{
    ...
    {
        var requirements = new List<IAuthorizationRequirement> { new FeatureRequirement("api-access") };
    ...
    public async Task ModuleDisabled_ShouldSucceed()
    {
        ...
        var handler = new RequiresFeatureAuthorizationHandler(BuildOptions(paymentsEnabled: false), service);
        var context = BuildContext(Guid.NewGuid());

        await handler.HandleAsync(context);

        context.HasSucceeded.Should().BeTrue();
        await service.DidNotReceive().UserHasFeatureAsync(Arg.Any<Guid>(), Arg.Any<string>(), Arg.Any<CancellationToken>());
    }
    ...
    {
        var service = Substitute.For<ISubscriptionAccessService>();
        var handler = new RequiresFeatureAuthorizationHandler(BuildOptions(paymentsEnabled: true), service);
    ...
    [Fact]
    public async Task UserWithFeature_ShouldSucceed()
    {
        var userId = Guid.NewGuid();
        var service = Substitute.For<ISubscriptionAccessService>();
        service.UserHasFeatureAsync(userId, Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(true);
    ...
    [Fact]
    public async Task UserWithoutFeature_ShouldNotSucceed()
    {
        var userId = Guid.NewGuid();
        var service = Substitute.For<ISubscriptionAccessService>();
        service.UserHasFeatureAsync(userId, Arg.Any<string>(), Arg.Any<CancellationToken>())
